...
This project is configured for Vercel, Neon Postgres, and Vercel Blob. Local development continues to use the SQLite database in `.db` when no Postgres URL is present.

1. Import the repository into a Vercel Hobby project.
2. Add a Neon database from the Vercel Marketplace. Ensure that it provides either `POSTGRES_URL` or a PostgreSQL `DATABASE_URL` to the project.
3. Create and connect a Vercel Blob store. Vercel will add `BLOB_READ_WRITE_TOKEN` automatically.
4. Add `PAYLOAD_SECRET`, `CRON_SECRET`, and `PREVIEW_SECRET` as long, random environment variables.
5. Deploy. `vercel.json` runs `pnpm ci`, which applies the migrations in `src/migrations-postgres` before building the app.
6. Open `/admin` on the deployed URL and create the first administrator.

The deployment intentionally fails when a Vercel build is missing Postgres or Blob credentials. This prevents Payload from writing production content to Vercel's ephemeral filesystem.

Media uploads use direct client uploads to bypass Vercel's server upload limit. The local `public/media` directory remains available only when Blob storage is disabled.

To develop against Neon instead of SQLite, copy the Neon connection string into `.env`:

```dotenv
POSTGRES_URL=postgresql://USER:PASSWORD@HOST:5432/DATABASE?sslmode=require
```

The first Postgres deployment starts with an empty database. The existing local SQLite database is not uploaded to Neon.
