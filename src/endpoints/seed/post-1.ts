...
export const createSeedAIDocument = (title: string) => ({
  compatibility: 'Payload CMS 示例站点',
  kind: 'reference' as const,
  markdown: `# 目标

帮助 AI 快速理解《${title}》的主题、范围和核心信息。
...
- 本文为演示内容，不能作为真实事实或专业建议使用。
- 引用结论前应使用可靠来源重新核验。

# 输出结果

生成一份明确标注为演示内容的主题摘要或研究提纲。

# 验证方法

检查生成结果是否明确标注本文为演示内容，并且没有把示例信息表述为已经核验的事实。
`,
...
export const post1: (args: PostArgs) => RequiredDataFromCollectionSlug<'posts'> = ({
  ...
  return {
    ...
    _status: 'published',
    aiDocument: createSeedAIDocument('Digital Horizons: A Glimpse into Tomorrow'),
    authors: [author],
    content: {
      root: {
        type: 'root',
